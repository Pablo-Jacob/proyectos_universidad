...
begin
    ...
    while i < product_count do
        ...
        call insert_salida(v_id_venta, v_id_producto);
...
begin
    ...
    select ve.id_venta, date_format(ve.fecha, '%Y-%m-%d') as fecha, ve.total, date_format((select max(fecha_hora)
		from bitacora_ventas_enc where id_venta = p_id_venta), '%Y-%m-%d %H:%i') as ultima_modificacion from ventas_enc ve
    ...
    select vd.id_det, vd.id_producto, p.descripcion as producto_descripcion, vd.cantidad, vd.iva,
        ...
        ((vd.precio_venta * vd.cantidad) + (vd.iva * vd.cantidad)) as total_item from ventas_det vd
...
begin
	select ve.id_venta, ve.fecha, date_format(max(ve.fecha), '%Y-%m-%d') as fecha, ve.total,
		date_format(max(bv.fecha_hora), '%Y-%m-%d %H:%i') as ultima_modificacion from ventas_enc ve
		left join ventas_det vd on ve.id_venta = vd.id_venta_det left join bitacora_ventas_enc bv on
        ve.id_venta = bv.id_venta where(p_fecha_inicio is null or ve.fecha >= p_fecha_inicio) and
        (p_fecha_fin is null or ve.fecha <= p_fecha_fin) group by ve.id_venta, ve.fecha, ve.total
